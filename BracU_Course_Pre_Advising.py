...
# --------------------- classes ----------------------#
class Subject:
    def __init__(self, name, faculty, section, totalSeat, seatBooked, seatAvailable, day1, day2, lab1, lab2,
                 classTime_start, classTime_end, labTime1_start, labTime2_start, labTime1_end, labTime2_end,
                 classRoomNumber, labRoomNumber):
        self.name = name
...
if internetAvailable() == False:
    ...
    for tr in rawData:
        ...
        def stringSeparator(timeSlots):
            ...
            return (lab1, lab2, labTime1_start, labTime1_end, labTime2_start, labTime2_end, labRoomNumber, day1, day2,
                    classTime_start, classTime_end, classRoomNumber)
        ...
        if len(timeSlots) == 1:
            ...
            lab1, lab2, labTime1_start, labTime1_end, labTime2_start, labTime2_end, labRoomNumber, day1, day2, classTime_start, classTime_end, classRoomNumber = stringSeparator(
                timeSlots)

        obj = Subject(name, faculty, section, totalSeat, seatBooked, seatAvailable, day1, day2, lab1, lab2,
                      classTime_start, classTime_end, labTime1_start, labTime2_start, labTime1_end, labTime2_end,
                      classRoomNumber, labRoomNumber)
    ...
    timeNameList = ['08:00 AM\n08:20 AM', '09:30 AM\n10:50 AM', '11:00 AM\n12:20 PM', '12:30 PM\n01:50 PM',
                    '02:00 PM\n03:20 PM', '03:30 PM\n04:50 PM', '05:00 PM\n06:20 PM']
    table = {
        'Saturday': {'08:00 AM': [], '09:30 AM': [], '11:00 AM': [], '12:30 PM': [], '02:00 PM': [], '03:30 PM': [],
                     '05:00 PM': []},
        'Sunday': {'08:00 AM': [], '09:30 AM': [], '11:00 AM': [], '12:30 PM': [], '02:00 PM': [], '03:30 PM': [],
                   '05:00 PM': []},
        'Monday': {'08:00 AM': [], '09:30 AM': [], '11:00 AM': [], '12:30 PM': [], '02:00 PM': [], '03:30 PM': [],
                   '05:00 PM': []},
        'Tuesday': {'08:00 AM': [], '09:30 AM': [], '11:00 AM': [], '12:30 PM': [], '02:00 PM': [], '03:30 PM': [],
                    '05:00 PM': []},
        'Wednesday': {'08:00 AM': [], '09:30 AM': [], '11:00 AM': [], '12:30 PM': [], '02:00 PM': [], '03:30 PM': [],
                      '05:00 PM': []},
        'Thursday': {'08:00 AM': [], '09:30 AM': [], '11:00 AM': [], '12:30 PM': [], '02:00 PM': [], '03:30 PM': [],
                     '05:00 PM': []},
        'Friday': {'08:00 AM': [], '09:30 AM': [], '11:00 AM': [], '12:30 PM': [], '02:00 PM': [], '03:30 PM': [],
                   '05:00 PM': []},
    }
    for r in range(totalRow):
        for c in range(totalColumn):
            if r == 0:
                lbl = Label(frame, text=dayNameList[c], font=("Ariel", 10, 'bold'), fg='#b8b8b8', bg='#5A5A5A',
                            width=13, height=2, borderwidth=1, relief='solid', padx=0, pady=0)
            elif r > 0 and c == 0:
                lbl = Label(frame, text=timeNameList[r - 1], font=("Ariel", 10, 'bold'), fg='#b8b8b8', bg='#5A5A5A',
                            width=13, height=3, borderwidth=1, relief='solid', padx=0, pady=0)
            else:
                lbl = Label(frame, text="", font=("Ariel", 10, "bold"), fg='white', bg='#5A5A5A', width=13, height=3,
                            borderwidth=1, relief='solid', padx=0, pady=0)
    ...
    def update_table(selectedCourses):
        ...
        for course in selectedCourses:
            if len(table[course.day1][course.classTime_start][0].cget('text')) < 1 and (
                    table[course.day1][course.classTime_start][0].cget('text') != f'{course.name} - Theory'):
                table[course.day1][course.classTime_start][0].config(text=f'{course.name} - Theory')
            elif len(table[course.day1][course.classTime_start][0].cget('text')) > 1 and (
                    table[course.day1][course.classTime_start][0].cget('text') != f'{course.name} - Theory'):
                st = table[course.day1][course.classTime_start][0].cget('text')
                st += f'\n{course.name} - Theory'
                table[course.day1][course.classTime_start][0].config(text=st, fg='red')

            if len(table[course.day2][course.classTime_start][0].cget('text')) < 1 and (
                    table[course.day2][course.classTime_start][0].cget('text') != f'{course.name} - Theory'):
                table[course.day2][course.classTime_start][0].config(text=f'{course.name} - Theory')
            elif len(table[course.day2][course.classTime_start][0].cget('text')) > 1 and (
                    table[course.day2][course.classTime_start][0].cget('text') != f'{course.name} - Theory'):
            ...
            if len(course.lab1) > 2:
                if len(table[course.lab1][course.labTime1_start][0].cget('text')) < 1 and (
                        table[course.lab1][course.labTime1_start][0].cget('text') != f'{course.name} - LAB'):
                    table[course.lab1][course.labTime1_start][0].config(text=f'{course.name} - LAB', fg='#a3bfff')
                elif len(table[course.lab1][course.labTime1_start][0].cget('text')) > 1 and (
                        table[course.lab1][course.labTime1_start][0].cget('text') != f'{course.name} - LAB'):
                    st = table[course.lab1][course.labTime1_start][0].cget('text')
                    st += f'\n{course.name} - LAB'
                    table[course.lab1][course.labTime1_start][0].config(text=st, fg='red')

                if len(table[course.lab2][course.labTIme2_start][0].cget('text')) < 1 and (
                        table[course.lab2][course.labTIme2_start][0].cget('text') != f'{course.name} - LAB'):
                    table[course.lab2][course.labTIme2_start][0].config(text=f'{course.name} - LAB', fg='#a3bfff')
                elif len(table[course.lab2][course.labTIme2_start][0].cget('text')) > 1 and (
                        table[course.lab2][course.labTIme2_start][0].cget('text') != f'{course.name} - LAB'):
    ...
    # Add
    addButton = Button(mainFrame, text='Add', command=addBut, font=('Ariel', 15, 'bold'), width=6, fg='green',
                       bg='#5A5A5A', padx=0, pady=0)
    addButton.grid(row=2, column=2, sticky='n')
    # Remove
    removeButton = Button(mainFrame, text='Remove', command=removeBut, font=('Ariel', 15, 'bold'), width=6, fg='red',
                          bg='#5A5A5A', padx=0, pady=0)
    removeButton.grid(row=2, column=2, sticky='w')

    # clear
    removeButton = Button(mainFrame, text='Clear', command=clearBut, font=('Ariel', 15, 'bold'), width=6, fg='black',
                          bg='#5A5A5A', padx=0, pady=0)
    ...
    Label(details, text=f'Course Initial:', font=('Arial', 13), width=20, fg='yellow', borderwidth=1, relief='solid',
          padx=0, pady=0, bg='grey').grid(row=2, column=0, sticky='w')
    Label(details, text=f'Faculty:', font=('Arial', 13), width=20, fg='yellow', borderwidth=1, relief='solid', padx=0,
          pady=0, bg='grey').grid(row=3, column=0, sticky='w')
    Label(details, text=f'Section:', font=('Arial', 13), width=20, fg='yellow', borderwidth=1, relief='solid', padx=0,
          pady=0, bg='grey').grid(row=4, column=0, sticky='w')
    Label(details, text=f'Total Seat:', font=('Arial', 13), width=20, fg='yellow', borderwidth=1, relief='solid',
          padx=0, pady=0, bg='grey').grid(row=5, column=0, sticky='w')
    Label(details, text=f'Seat Booked:', font=('Arial', 13), width=20, fg='yellow', borderwidth=1, relief='solid',
          padx=0, pady=0, bg='grey').grid(row=6, column=0, sticky='w')
    Label(details, text=f'Free Seat:', font=('Arial', 13), width=20, fg='yellow', borderwidth=1, relief='solid', padx=0,
          pady=0, bg='grey').grid(row=7, column=0, sticky='w')
    Label(details, text=f'Theory Class:', font=('Arial', 13), width=20, fg='yellow', borderwidth=1, relief='solid',
          padx=0, pady=0, bg='grey').grid(row=8, column=0, sticky='w')
    Label(details, text=f'Theory Class Time:', font=('Arial', 13), width=20, fg='yellow', borderwidth=1, relief='solid',
          padx=0, pady=0, bg='grey').grid(row=9, column=0, sticky='w')
    Label(details, text=f'Class Room:', font=('Arial', 13), width=20, fg='yellow', borderwidth=1, relief='solid',
          padx=0, pady=0, bg='grey').grid(row=10, column=0, sticky='w')
    Label(details, text=f'Lab Class:', font=('Arial', 13), width=20, fg='yellow', borderwidth=1, relief='solid', padx=0,
          pady=0, bg='grey').grid(row=11, column=0, sticky='w')
    Label(details, text=f'Theory Class Time:', font=('Arial', 13), width=20, fg='yellow', borderwidth=1, relief='solid',
          padx=0, pady=0, bg='grey').grid(row=12, column=0, sticky='w')
    Label(details, text=f'Lab Room:', font=('Arial', 13), width=20, fg='yellow', borderwidth=1, relief='solid', padx=0,
          pady=0, bg='grey').grid(row=13, column=0, sticky='w')

    label1 = Label(details, text=f'', font=('Arial', 13), width=20, fg='#fc03f0', borderwidth=1, relief='solid', padx=0,
                   pady=0, bg='grey')
    label1.grid(row=2, column=1, sticky='w')
    label2 = Label(details, text=f'', font=('Arial', 13), width=20, fg='#fc03f0', borderwidth=1, relief='solid', padx=0,
                   pady=0, bg='grey')
    label2.grid(row=3, column=1, sticky='w')
    label3 = Label(details, text=f'', font=('Arial', 13), width=20, fg='#fc03f0', borderwidth=1, relief='solid', padx=0,
                   pady=0, bg='grey')
    label3.grid(row=4, column=1, sticky='w')
    label4 = Label(details, text=f'', font=('Arial', 13), width=20, fg='#fc03f0', borderwidth=1, relief='solid', padx=0,
                   pady=0, bg='grey')
    label4.grid(row=5, column=1, sticky='w')
    label5 = Label(details, text=f'', font=('Arial', 13), width=20, fg='#fc03f0', borderwidth=1, relief='solid', padx=0,
                   pady=0, bg='grey')
    label5.grid(row=6, column=1, sticky='w')
    label6 = Label(details, text=f'', font=('Arial', 13), width=20, fg='#fc03f0', borderwidth=1, relief='solid', padx=0,
                   pady=0, bg='grey')
    label6.grid(row=7, column=1, sticky='w')
    label7 = Label(details, text=f'', font=('Arial', 13), width=20, fg='#fc03f0', borderwidth=1, relief='solid', padx=0,
                   pady=0, bg='grey')
    label7.grid(row=8, column=1, sticky='w')
    label8 = Label(details, text=f'', font=('Arial', 13), width=20, fg='#fc03f0', borderwidth=1, relief='solid', padx=0,
                   pady=0, bg='grey')
    label8.grid(row=9, column=1, sticky='w')
    label9 = Label(details, text=f'', font=('Arial', 13), width=20, fg='#fc03f0', borderwidth=1, relief='solid', padx=0,
                   pady=0, bg='grey')
    label9.grid(row=10, column=1, sticky='w')
    label10 = Label(details, text=f'', font=('Arial', 13), width=20, fg='#fc03f0', borderwidth=1, relief='solid',
                    padx=0, pady=0, bg='grey')
    label10.grid(row=11, column=1, sticky='w')
    label11 = Label(details, text=f'', font=('Arial', 13), width=20, fg='#fc03f0', borderwidth=1, relief='solid',
                    padx=0, pady=0, bg='grey')
    label11.grid(row=12, column=1, sticky='w')
    label12 = Label(details, text=f'', font=('Arial', 13), width=20, fg='#fc03f0', borderwidth=1, relief='solid',
                    padx=0, pady=0, bg='grey')
    ...
    detailsButton = Button(details, text='Show Details', command=detailsBut, font=('Ariel', 15, 'bold'), width=12,
                           fg='#5c0049', bg='#5A5A5A', padx=0, pady=0)
